...
export async function createAnalysis(req: AuthRequest, res: Response): Promise<void> {
  ...
  const { babyId, label, confidence, durationSec, notes } = parsed.data;
  ...
  const analysis = await prisma.cryAnalysis.create({
    data: { babyId, label: label as CryLabel, confidence, durationSec, notes },
  });
...
export async function getHistory(req: AuthRequest, res: Response): Promise<void> {
  ...
  // Verify the baby belongs to this user if babyId is provided
  if (babyId) {
    const baby = await prisma.baby.findUnique({ where: { id: babyId } });
    if (!baby || baby.userId !== req.userId) {
      res.status(404).json({ error: 'Baby not found' });
  ...
  // Collect all babyIds belonging to the user when no filter is given
  const where = babyId
    ? { babyId }
    : {
